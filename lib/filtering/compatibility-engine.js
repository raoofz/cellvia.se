(() => {
  ...
  const scoreByStatus = {
    ...
    "Begränsad": 35,
    "Ej rekommenderad": 0,
    "Kan vara tillåten": 82,
    "Behöver kontrolleras": 70,
    "Restricted": 35,
    "Not recommended": 0,
    "Inte tillåten": -10
  };
  ...
  function evaluateProductForPrison(product, prison) {
    if (!prison) {
      return {
        ...
        hiddenByDefault: product.compatibilityStatus === "Ej rekommenderad" || product.compatibilityStatus === "Inte tillåten",
    ...
    if (categoryRestricted || prison.electronicsPolicy === "restricted" && /Elektronik|Hörlurar|Batterier|Musik/.test(product.category)) {
    ...
    if (!explicitlyCompatible && product.requiresManualReview) {
      ...
      status = status === "Ej rekommenderad" || status === "Inte tillåten" ? status : "Kräver kontroll";
    ...
    if (status === "Inte tillåten") {
      score = -10;
      warnings.push("Produkten bör inte ingå utan direkt skriftlig kontroll med anstalten.");
    }
