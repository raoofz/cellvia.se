(() => {
  ...
  function filterProducts(products, { query = "", category = "", prisonId = "", status = "", packageTag = "", sourceCategory = "", availability = "" }, compatibility) {
    return products.filter((product) => {
      const matchesText = includesText(product, query, ["name", "description", "category", "catalogCategory", "sourceCategory", "usefulFor", "compatibilityNote"]);
      const matchesCategory = !category || product.category === category || product.catalogCategory === category;
      ...
      const matchesSource = !sourceCategory || product.sourceCategory === sourceCategory;
      const matchesAvailability = !availability || product.stockStatus === availability;
      return matchesText && matchesCategory && matchesPrison && matchesStatus && matchesPackage && matchesSource && matchesAvailability;
    });
  }
