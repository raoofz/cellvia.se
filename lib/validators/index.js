(() => {
  function validateOrder(payload) {
    ...
    if (!payload.inmateNumber || payload.inmateNumber.trim().length < 2) errors.push("Ange klientnummer eller intagningsnummer för den intagne.");
    if (!payload.prisonId) errors.push("Välj anstalt.");
    if (!payload.customerName || payload.customerName.trim().length < 2) errors.push("Ange kundens namn.");
